...
def count_neighbour_vector(vector, count_value, is_output_str=False):
    vector = list(vector)
    neighbour_counts = []
    count = 0
    for v in vector:
...
class ConsoleGame:
    ...
    def check_rule(self, input_neighbours, label_neighbours):
        if sum(input_neighbours) > sum(label_neighbours):
            return False
        return input_neighbours <= label_neighbours
    ...
    def count_neighbour_vector(self, vector, count_value, is_output_str=False):
        ...
        return count_neighbour_vector(vector, count_value, is_output_str)
